...
def deletar_flashcard(request, id):
    ...
    if request.user != flashcard.user:
        ...
        return redirect('flashcard:novo_flashcard')
    flashcard_desafios = FlashcardDesafio.objects.filter(flashcard=flashcard)
    flashcard_desafios.delete()

    flashcard.delete()
    messages.add_message(
        request, constants.WARNING, 'Flashcard deletado com sucesso!'
    )
    return redirect('flashcard:novo_flashcard')
...
def iniciar_desafio(request):
    if request.method == 'GET':
        ...
        if flashcards_count < int(qtd_perguntas):
            messages.warning(
                request,
                f'Atenção: Você possui apenas {flashcards_count} flashcards disponíveis. O desafio será criado com essa quantidade.',  # NOQA
            )
...
def desafio(request, id):
    ...
    if request.method == 'GET':
        flash = FlashcardDesafio.objects.filter(
            desafio=desafio, respondido=False).first()
        acertos = (
